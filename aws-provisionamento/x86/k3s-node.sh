...
aws ec2 run-instances \
    ...
    --user-data file://scripts/k3s-node.sh \
    --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=k3s-node}]' 'ResourceType=volume,Tags=[{Key=Name,Value=k3s-node}]' \
    --output table